...
There are for types reactions of Bossman can have for shown events:
	a) win [currently 8 different clips],
	b) loss [currently 5 different clips],
	c) beg [currently 5 different clips],
	d) rage [currently 8 different clips],
	e) bigwin [currently 3 different clips].
...
As for today, there are implemented following games:
	...
	4) Coin Flip - game taken straight from BC Games - little more rigged and deinetely not mathematically abusive.
	5) Dice      - press a button and check if you will win. Little more sophisticated version of YesNo.

-----------
Also project allows to see the source of the reaction (just by clikcing the Austin portrair in the end of animation). In order
to interrupt the hero animation, just click the button "Slake.com" in thi top of the screen. 
...
files in the same directory where there is stored .xlsm file, and you are ready to run.
